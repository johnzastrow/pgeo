...
| 2026-09-18 | Venue/lake failure analysis. (1) The bare town is a name target only when no name part was parsed; (2) the queried town is resolved to its location(s), which anchor same-name tie-breaks when there is no focus and count as town agreement by distance for non-address results (exp(-d/6 km)); (3) an admin result auto-agrees only for bare-town queries; (4) ambiguity: when a town is among the ties, only other towns count as rivals | 36 of 47 venue/lake exact failures shared two causes: "Subway, Cumberland Mills" matched the town exactly (conf 1.0) instead of the venue, and the query's town is often a village or nearest town, not the containing one ("Calvary Bible Church, Stratton" is in Eustis). Bare towns ("Portland, Maine") scored 0.59 because same-named venues counted as rivals | pure SQL 93.1% -> **95.8%**: venues exact 148 -> 172 of 183, venue typos 34 -> 40 of 42, lakes/summits exact 71 -> 79 of 83; towns -2 (two Maine names that exist twice, coin flips). Fuzz F0-F5 **98.0** / 87.0 / 74.3 / 46.0 / 45.0 / 49.0. FastAPI + rule parser identical (95.8%); libpostal service 93.9%. Confidence right/wrong 0.92 / 0.73 | p50 40 ms (unchanged) |
| 2026-09-18 | Address lookup also takes the house number on the 3 best street names in *any* town (index feature_addr_street_hn_idx), not only the 25 (street, town) pairs the street step keeps | "21 Church St, Maine": ~150 Church Streets, 25 kept arbitrarily, one had #21, so an ambiguous query looked unique at confidence 1.0 | 95.8% (addresses exact +1); confidence right/wrong 0.92 / **0.69** (was 0.73); fuzz unchanged | none |

Remaining failures (66 of 1,560) after this round:

- 8 of the 150 "misses" exist in Maine ("Death Valley" in Minot x7, "The Eiffel Tower of Paris, Maine"):
  test-set errors, to be removed from the miss list when both engines are re-run.
- Duplicate chain branches and duplicate place names (Yianni's House of Pizza, two Higgins
  Corners, 04732 in two towns): genuinely ambiguous; confidence is low, as intended.
- Typos in both the name and the town (14 lake/summit typo cases): the hardest class.
- 6 autocomplete venue cases: the right venue is outside the top 5 of short prefixes.
