...
pgeo 0.7.0, project 0.12.2, commit `0de02be (with uncommitted changes)` ·
...
- **Autocomplete speed**: with reverse geocoding fixed, autocomplete is the endpoint that decides
  pgeo's capacity (Figure 14), and it has the tightest target. Profile the
...
- **Shadow comparison on real traffic**: keep Pelias beside pgeo on the query host for a few weeks,
  ...
  (freeing its 7.17 GB on the host). This is also the only way to settle the
...
**Table 39. Phonetic misspellings against the deployed pgeo.** All five resolve, but two are weak: "Kenebunc" is found with low confidence, and "Skowhegin" ranks a venue above the town.
...
**Table 40. How coarse each phonetic encoding is on this data.** Soundex puts 18 distinct towns -- North Bancroft, Northport, North Haven, Northfield and fourteen more -- into one bucket.
...
**Table 41. Questions asked during the project, with short answers and the sections that answer them in full.**
...
**Table 42. pgeo tables (including their indexes).**
