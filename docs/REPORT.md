...
John Zastrow · 2026-09-20

pgeo 0.7.0, project 0.12.2, commit `2c3c563 (with uncommitted changes)` ·
...
**Table 31. Compatibility contract cases.**
...
**Table 32. Feature parity through the documented Pelias API.**
...
**Table 33. Accuracy parity by category:** Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
**Table 34. Throughput parity:** most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.
...
**Table 35. Both engines on VM 120 through the production HTTPS path:** the three-user validation, the p95 per endpoint and how far the ramp went.
...
conditions (Table 34). Both engines do better on the VM than on the
...
it will fail too. Table 36 is the whole path, pass and fail.

**Table 36. Minimum-server search:** every trial, in order, for both engines. Memory totals include 0.8 GB for the operating system.
...
**Table 37. The measured floor confirmed on a temporary VM:** the three-user validation and the ramp to the first configuration outside a latency target.
...
**Table 38. Security properties of each platform as deployed.** Measured 2026-09-20.
...
**Table 39. Which tool for which scenario.**
...
**Table 40. Work not done, with the effort, the likelihood it pays off and the value of finishing it.**
...
**Table 41. Phonetic misspellings against the deployed pgeo.** All five resolve, but two are weak: "Kenebunc" is found with low confidence, and "Skowhegin" ranks a venue above the town.
...
**Table 42. How coarse each phonetic encoding is on this data.** Soundex puts 18 distinct towns -- North Bancroft, Northport, North Haven, Northfield and fourteen more -- into one bucket.
...
**Table 43. Questions asked during the project, with short answers and the sections that answer them in full.**
...
**Table 44. pgeo tables (including their indexes).**
