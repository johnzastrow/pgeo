...
pgeo 0.7.0, project 0.12.2, commit `77728cc (with uncommitted changes)` ·
...
| pgGeocoder[8] | entirely PostgreSQL functions | Japanese government address datasets (ISJ, KSJ) | SQL only: `geocoder('...')` and `reverse_geocoder(lon, lat)` | The closest architectural sibling found: all logic in the database, forward and reverse, PostGIS only. Japan-only, and no HTTP layer |
| TIGER packagings[9, 10] | none of their own | TIGER | `psql`; one adds a Python API and Docker | Two of them, both wrapping the PostGIS TIGER geocoder rather than adding matching; both quiet since 2022 |
| Nominatim[11] | indexing and address computation in PL/pgSQL triggers; **search in a Python application** | OpenStreetMap | HTTP API | Postgres-centric rather than Postgres-only. Version 5 completed the move from PHP to Python. Planet-scale: ~1 TB of disk and 128 GB-class memory; the 2025 roadmap[12] notes search indexes have grown large enough to slow lookups |
...
such as OpenCage's[13] also lets you geocode "from SQL", but by calling a remote
...
| **libpostal**[14] | A statistical address parser (a ~2 GB model) used by Pelias and optionally by pgeo |
| **Elasticsearch** | The search engine Pelias stores its index in |
| **k6**[15] | The load-testing tool used throughout: it runs scripted virtual users against an HTTP API and reports latency percentiles and error rates (`tests/load/k6`) |
...
| **Trigram** | A three-character slice of a word[16]. Comparing the sets of trigrams in two strings measures similarity, which is how pgeo tolerates typos (`pg_trgm`) |
| **WOF** | Who's On First[17], the open gazetteer of administrative places used for towns, counties and neighbourhoods |
| **OA** | OpenAddresses[18], the open collection of address points |
| **GNIS** | The USGS Geographic Names Information System[19]: lakes, summits, streams and populated places |
| **ZCTA** | ZIP Code Tabulation Area[20], the US Census approximation of a ZIP code as an area |
...
build (`pgeo-load build`) loads the same inputs with DuckDB[21], ogr2ogr[22] and COPY, assigns each
...
pgeo's `/v1/address` returns a US address in USPS Publication 28[23] form (number, directionals,
...
The USPS ZIP+4 file[24] would add ZIP+4 codes, USPS street spellings and the USPS preferred
...
8. pgGeocoder: a Japanese geocoder implemented entirely in PostgreSQL functions. https://github.com/mbasa/pgGeocoder
9. moofish32/postgis-geocoder: a Docker packaging of the PostGIS TIGER geocoder. https://github.com/moofish32/postgis-geocoder
10. MattTriano/postgis_geocoder: a Docker and Python packaging of the PostGIS TIGER geocoder. https://github.com/MattTriano/postgis_geocoder
11. Nominatim architecture overview, release 5.3 documentation. https://nominatim.org/release-docs/latest/develop/overview/
12. The road to Nominatim 6, July 2025. https://nominatim.org/2025/07/14/roadmap-nominatim6.html
13. OpenCage geocoding foreign data wrapper for PostgreSQL (calls a remote API from SQL). https://blog.opencagedata.com/post/geocoding-in-postgresql
14. libpostal: a C library for parsing and normalising street addresses, using statistical NLP. https://github.com/openvenues/libpostal
15. Grafana k6: a load-testing tool with scripted virtual users. https://k6.io
16. pg_trgm: trigram-based text similarity and indexing. PostgreSQL documentation. https://www.postgresql.org/docs/18/pgtrgm.html
17. Who's On First: an open gazetteer of places. https://whosonfirst.org
18. OpenAddresses: a global collection of open address data. https://openaddresses.io
19. USGS Geographic Names Information System (GNIS), Domestic Names. https://www.usgs.gov/us-board-on-geographic-names
20. US Census Bureau, ZIP Code Tabulation Areas (ZCTA) gazetteer. https://www.census.gov/geographies/reference-files/time-series/geo/gazetteer-files.html
21. DuckDB, used in the pgeo build to read Parquet and CSV sources. https://duckdb.org
22. GDAL/OGR (ogr2ogr), used in the pgeo build to load spatial formats. https://gdal.org
23. United States Postal Service, Publication 28: Postal Addressing Standards. https://pe.usps.com/text/pub28/welcome.htm
24. United States Postal Service, ZIP+4 product and licence terms. https://postalpro.usps.com/address-quality/zip4
