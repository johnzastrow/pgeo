...
pgeo 0.7.0, project 0.12.2, commit `7f7fcaa (with uncommitted changes)` ·
...
pgeo on every configuration tested (Figure 14). The comparison that would press
hardest on pgeo's weakest axis is the one this study does not make.


> **Caution.** Pelias is the right primary comparison and not a sufficient one. The honest limitation is that pgeo's binding endpoint is autocomplete, and the specialist at autocomplete is Photon, which was not tested. A reader should treat "four times fewer users per CPU than Pelias" as the cost against a general-purpose multi-source geocoder, not as pgeo's standing against the fastest available type-ahead engine.
...
**Table 45. Which tool for which scenario.**
...
**Table 46. Work not done, with the effort, the likelihood it pays off and the value of finishing it.**
...
**Table 47. Phonetic misspellings against the deployed pgeo.** All five resolve, but two are weak: "Kenebunc" is found with low confidence, and "Skowhegin" ranks a venue above the town.
...
**Table 48. How coarse each phonetic encoding is on this data.** Soundex puts 18 distinct towns -- North Bancroft, Northport, North Haven, Northfield and fourteen more -- into one bucket.
...
**Table 49. Questions asked during the project, with short answers and the sections that answer them in full.**
...
**Table 50. pgeo tables (including their indexes).**
