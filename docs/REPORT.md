...
`b3fecce (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260919-1135-pgeo, 20260919-1543-pgeo.
...
435 MB Elasticsearch snapshot or a PostgreSQL dump. Table 23 in Section 3.5
...
conditions (Table 30). Both engines do better on the VM than on the
...
> **Key result.** The three-user service fits in 1.40 GB on pgeo and 7.17 GB on Pelias -- a factor of five. Both engines idle comfortably below a quarter of a core, so at this scale the machine is chosen by memory, not by CPU -- which is why the two platforms land in different price classes (Table 24).
...
**Table 33.** The measured floor confirmed on a temporary VM: the three-user validation and the ramp to the first configuration outside a latency target.
...
**Table 34.** Security properties of each platform as deployed. Measured 2026-09-20.
...
**Table 35.** Which tool for which scenario.
...
**Table 36.** Questions asked during the project, with short answers and the sections that answer them in full.
...
**Table 37.** pgeo tables (including their indexes).
