...
`7167f31 (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260919-1135-pgeo, 20260919-1543-pgeo.
...
24 users there, where the same test needs about 8.3 GB for Pelias in its standard profile and [missing value floor_pelias_gb] once every service is squeezed to its floor (Section 3.12). A
...
[missing value floor_pelias_gb] floor needs the 8 GB tier, four to ten times the monthly price for the
...
server, where Pelias's four-fold throughput per CPU saves hardware once its [missing value floor_pelias_gb] memory floor is
...
memory, where Pelias needs six containers and [missing value floor_pelias_gb], which is the difference
...
- **Shadow comparison on real traffic**: keep Pelias beside pgeo on the query host for a few weeks,
  ...
  (freeing its [missing value floor_pelias_gb] on the host). This is also the only way to settle the
