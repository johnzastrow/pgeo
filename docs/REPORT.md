...
`82d416b (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260919-1135-pgeo, 20260919-1543-pgeo.
...
to 0.69 while right answers stayed high, which is what makes the confidence
score usable.



> **Key result.** One missing database index (the join from admin polygons to their features) made every reverse request scan all 906,101 features. Adding it cut reverse latency from 225-393 ms to 5-18 ms and raised capacity on one vCPU from 4 to 24 users: a six-fold gain from a single line of DDL, found only because the load tests measured each endpoint separately.
...
it will fail too. Table 31 is the whole path, pass and fail.

**Table 31.** Minimum-server search: every trial, in order, for both engines. Memory totals include 0.8 GB for the operating system.
...
> **Key result.** The three-user service fits in 1.40 GB on pgeo and [missing value floor_pelias_gb] on Pelias. Both engines idle comfortably below a quarter of a core, so at this scale the machine is chosen by memory, not by CPU -- which is why the two platforms land in different price classes ({{ref:table:vps
).}}
...
**Table 32.** The measured floors confirmed on temporary VMs: the three-user validation and the ramp to the first configuration outside a latency target.
...
**Table 33.** Which tool for which scenario.
...
**Table 34.** Questions asked during the project, with short answers and the sections that answer them in full.
...
**Table 35.** pgeo tables (including their indexes).
