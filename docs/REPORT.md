...
`5994034 (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260918-2343-pgeo, 20260919-0350-pgeo.
...
**Capacity.** Pelias is the faster engine: at equal CPU it keeps 4 to
24 times more concurrent users within the latency targets. pgeo is the smaller
one: it meets every target for the 3-user goal of this project in a
1.6 GB memory budget on one vCPU, where Pelias needs about
8.3 GB to run at all. <!-- PENDING: final pgeo capacity sentence after the post-fix run -->
...
| Build and test host | Workstation: AMD Ryzen 5 3600 6-Core Processor, 12 threads, 31 GB RAM |
...
**Table 2.** Data sources, raw input sizes, and what each engine holds from them. Pelias counts are Elasticsearch documents; pgeo counts are rows in its feature table.
...
**Table 4.** pgeo build steps and their duration (latest build).
...
**Table 5.** Composition of the accuracy test set by endpoint, category and query quality.
...
**Table 6.** Fuzz levels. House numbers are never corrupted, so every query stays answerable.
...
**Table 7.** Load-test query corpus (drawn at random per request).
...
**Table 8.** Example requests from the load session.
...
**Table 9.** Latency targets (95th percentile) used as the pass criterion.
...
**Table 10.** Tested configurations. pgeo configurations were run with each front end (rest = pure SQL through PostgREST, api = FastAPI); budgets include 0.8 GB for the operating system.
...
**Table 11.** Accuracy on 1,560 cases. Correct = right answer at rank 1 (misses: nothing, or low confidence); Hit@5 = right answer in the top five; confidence columns are the mean confidence of the first result when right and when wrong.
...
**Table 12.** Accuracy by category (all query qualities).
...
pgeo (Table 14). Eight "misses" are test-set errors (Death Valley in
...
**Table 14.** Remaining pgeo failures by category.
...
**Table 15.** Tuning changes, the problem each solved, and the measured effect.
...
**Table 16.** Median and p95 latency at 3 users (ms).
...
| rest-Pmin | 22 / 56 | 59 / 142 | 90 / 124 | 311 / 393 |
| rest-P1 | 16 / 42 | 50 / 101 | 103 / 197 | 321 / 348 |
| rest-P2 | 15 / 35 | 40 / 100 | 89 / 100 | 213 / 226 |
| rest-P4 | 15 / 40 | 37 / 107 | 93 / 116 | 190 / 225 |
| rest-PM | 14 / 42 | 38 / 106 | 91 / 116 | 235 / 255 |
| api-Pmin | 20 / 53 | 30 / 98 | 59 / 91 | 293 / 325 |
| api-P1 | 19 / 61 | 78 / 109 | 83 / 104 | 277 / 340 |
| api-P2 | 16 / 39 | 46 / 127 | 44 / 81 | 282 / 291 |
| api-P4 | 14 / 41 | 49 / 116 | 89 / 99 | 199 / 224 |
| api-PM | 15 / 39 | 48 / 89 | 93 / 106 | 232 / 257 |
| api-svc-P2 | 16 / 46 | 48 / 109 | 79 / 116 | 212 / 225 |
| api-svc-P4 | 17 / 45 | 49 / 108 | 99 / 106 | 209 / 232 |
...
**Table 17.** Pelias capacity by configuration.
...
**Table 18.** pgeo capacity by configuration (after the reverse-geocoding index fix).

| Config | vCPU | Budget | Settings | Users within SLO | Broken at | First over target | req/s at limit | Memory at 3 users |
|:--|--:|--:|:--|--:|--:|:--|--:|--:|
| rest-Pmin | 1 | 1.6 GB | sb 128MB, 4 conn. | 4 | 64 | reverse at 6 | 4 | 0.3 GB |
| rest-P1 | 1 | 2.0 GB | sb 256MB, 4 conn. | 4 | 64 | reverse at 6 | 4 | 0.4 GB |
| rest-P2 | 2 | 2.5 GB | sb 384MB, 6 conn. | 24 | 96 | reverse at 32 | 26 | 0.5 GB |
| rest-P4 | 4 | 3.0 GB | sb 512MB, 10 conn. | 64 | 192 | reverse at 96 | 73 | 0.5 GB |
| rest-PM | all | - | sb 1GB, 16 conn. | 128 | 384 | reverse at 192 | 145 | 0.6 GB |
| api-Pmin | 1 | 1.6 GB | sb 128MB, 4 conn. | 8 | 64 | reverse at 12 | 10 | 0.3 GB |
| api-P1 | 1 | 2.0 GB | sb 256MB, 4 conn. | 6 | 64 | reverse at 8 | 8 | 0.4 GB |
| api-P2 | 2 | 2.7 GB | sb 384MB, 6 conn. | 32 | 128 | reverse at 48 | 37 | 0.6 GB |
| api-P4 | 4 | 3.3 GB | sb 512MB, 10 conn. | 96 | 256 | reverse at 128 | 108 | 0.8 GB |
| api-PM | all | - | sb 1GB, 16 conn. | 192 | 512 | autocomplete, reverse at 256 | 218 | 0.8 GB |
| api-svc-P2 | 2 | 4.7 GB | sb 384MB, 6 conn. | 24 | 128 | reverse at 32 | 29 | 2.5 GB |
| api-svc-P4 | 4 | 5.3 GB | sb 512MB, 10 conn. | 32 | 32 | - | 29 | 2.7 GB |


**Table 19.** Effect of the reverse-geocoding index on pgeo capacity and reverse latency.

| Config | Users within SLO, before | after | Reverse p95 at 3 users (ms), before | after |
|:--|--:|--:|--:|--:|
| rest-Pmin | 4 | 4 | 393 | 393 |
| rest-P1 | 4 | 4 | 348 | 348 |
| rest-P2 | 24 | 24 | 226 | 226 |
| rest-P4 | 64 | 64 | 225 | 225 |
| rest-PM | 128 | 128 | 255 | 255 |
| api-Pmin | 8 | 8 | 325 | 325 |
| api-P1 | 6 | 6 | 340 | 340 |
| api-P2 | 32 | 32 | 291 | 291 |
| api-P4 | 96 | 96 | 224 | 224 |
| api-PM | 192 | 192 | 257 | 257 |
...
**Figure 16.** Peak memory in use by service at 3 users. Pelias carries the libpostal model (~2 GB), the interpolation database and Elasticsearch's heap regardless of load; pgeo's memory is mostly the PostgreSQL page cache for a 674 MB database.
...
**Operation.** Table Table 20 turns the capacity results into a sizing guide: for a
...
**Table 21.** Operating requirements of each platform (Maine data).
...
never needs the raw data or the build tools. Table Table 22 gives the measured
cost of a full build of each engine.

**Table 22.** Build resources: a full build of each engine, measured with `tests/build/measure_build.py` (wall time, peak memory of all build containers and processes, average and peak CPU, disk used by the result).
...
**Table 23.** Pelias by dataset: D1 Who's On First; D2 + OpenAddresses; D3 + OpenStreetMap; D4 + GNIS and ZCTA; D5 + Overture places (full).
...
**Table 24.** Compatibility contract cases.
...
| search boundary.rect | `/v1/search?text=main st&boundary.rect.min_lat=44.7&boundary.rect.max_lat=44.9&boundary.re` | pass | pass | pass |
| search boundary.circle | `/v1/search?text=main st&boundary.circle.lat=44.8016&boundary.circle.lon=-68.7712&boundary` | pass | pass | pass |
| search circle + focus | `/v1/search?text=main st&boundary.circle.lat=44.8016&boundary.circle.lon=-68.7712&boundary` | pass | pass | pass |
...
| structured circle | `/v1/search/structured?address=Main St&boundary.circle.lat=44.8016&boundary.circle.lon=-68` | pass | pass | pass |
| autocomplete | `/v1/autocomplete?text=389 congress` | pass | pass | pass |
| autocomplete circle | `/v1/autocomplete?text=main&boundary.circle.lat=44.8016&boundary.circle.lon=-68.7712&bound` | pass | pass | pass |
...
**Table 25.** Features of each platform as deployed here.
...
**Table 26.** Feature parity through the documented Pelias API.
...
**Table 27.** Accuracy parity by category: Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
| Reverse | 100% | 100% | -0 | Pelias |
| Misses | 41% | 95% | +53 | pgeo |
| All cases | 75.5% | 95.8% | +20.3 | pgeo |


**Table 28.** Throughput parity: most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.

| CPU | Pelias users (budget) | pgeo pure SQL | pgeo FastAPI | Pelias / best pgeo |
|:--|--:|--:|--:|--:|
| 1 vCPU | 96 (8.3 GB) | 4 (2.0 GB) | 6 (2.0 GB) | 16.0x |
| 2 vCPU | 192 (8.4 GB) | 24 (2.5 GB) | 32 (2.7 GB) | 6.0x |
| 4 vCPU | 384 (10.8 GB) | 64 (3.0 GB) | 96 (3.3 GB) | 4.0x |
...
**Table 29.** Which tool for which scenario.
...
**Table 30.** pgeo tables (including their indexes).
