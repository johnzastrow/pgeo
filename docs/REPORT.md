...
`08715ea (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260918-2343-pgeo, 20260919-0350-pgeo.
...
**Table 20.** Pelias by dataset: D1 Who's On First; D2 + OpenAddresses; D3 + OpenStreetMap; D4 + GNIS and ZCTA; D5 + Overture places (full).
...
**Table 21.** Compatibility contract cases.
...
**Table 22.** Features of each platform as deployed here.
...
**Table 23.** Feature parity through the documented Pelias API.
...
**Table 24.** Accuracy parity by category: Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
**Table 25.** Throughput parity: most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.
...
**Table 26.** Which tool for which scenario.
...
**Table 27.** pgeo tables (including their indexes).
