...
`751c686 (with uncommitted changes)` · data snapshot 2026-09-19 · load-test runs 20260918-1834, 20260918-2051, 20260919-0718, 20260919-1135-pgeo, 20260919-1543-pgeo.
...
large (Table Table 8) and drawn at random, so exact repeats are rare.
...
24): the working set of hot index pages is far smaller than the 674 MB database.
...
**Operation.** Table Table 21 turns the capacity results into a sizing guide: for a
...
never needs the raw data or the build tools. Table Table 23 gives the measured
...
**Table 24.** Pelias by dataset: D1 Who's On First; D2 + OpenAddresses; D3 + OpenStreetMap; D4 + GNIS and ZCTA; D5 + Overture places (full).
...
**Table 25.** Compatibility contract cases.
...
**Table 26.** Features of each platform as deployed here.
...
**Table 27.** Feature parity through the documented Pelias API.
...
**Table 28.** Accuracy parity by category: Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
**Table 29.** Throughput parity: most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.

| CPU | Pelias users (budget) | pgeo pure SQL | pgeo FastAPI | Pelias / best pgeo |
|:--|--:|--:|--:|--:|
| 1 vCPU | 96 (8.3 GB) | 24 (2.0 GB) | 24 (2.0 GB) | 4.0x |
...
**Table 30.** Which tool for which scenario.
...
**Table 31.** Questions asked during the project, with short answers and the sections that answer them in full.
...
**Table 32.** pgeo tables (including their indexes).
