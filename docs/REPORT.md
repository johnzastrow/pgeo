...
pgeo 0.7.0, project 0.12.2, commit `36590a1 (with uncommitted changes)` ·
...
435 MB Elasticsearch snapshot or a PostgreSQL dump. Table 27 in Section 3.5
...
Table 25 turns the capacity results into a sizing guide: for a
...
**Table 26. Operating requirements of each platform (Maine data).**
...
never needs the raw data or the build tools. Table 27 gives the measured
cost of a full build of each engine.

**Table 27. Build resources:** a full build of each engine, measured with `tests/build/measure_build.py` (wall time, peak memory of all build containers and processes, average and peak CPU, disk used by the result).
...
**Table 29. Pelias by dataset:** D1 Who's On First; D2 + OpenAddresses; D3 + OpenStreetMap; D4 + GNIS and ZCTA; D5 + Overture places (full).
...
**Table 30. Compatibility contract cases.**
...
**Table 31. Feature parity through the documented Pelias API.**
...
**Table 32. Accuracy parity by category:** Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
**Table 33. Throughput parity:** most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.
...
**Table 34. Both engines on VM 120 through the production HTTPS path:** the three-user validation, the p95 per endpoint and how far the ramp went.
...
conditions (Table 33). Both engines do better on the VM than on the
...
it will fail too. Table 35 is the whole path, pass and fail.

**Table 35. Minimum-server search:** every trial, in order, for both engines. Memory totals include 0.8 GB for the operating system.
...
> **Key result.** The three-user service fits in 1.40 GB on pgeo and 7.17 GB on Pelias -- a factor of five. Both engines idle comfortably below a quarter of a core, so at this scale the machine is chosen by memory, not by CPU -- which is why the two platforms land in different price classes (Table 28).
...
**Table 36. The measured floor confirmed on a temporary VM:** the three-user validation and the ramp to the first configuration outside a latency target.
...
**Table 37. Security properties of each platform as deployed.** Measured 2026-09-20.
...
**Table 38. Which tool for which scenario.**
...
**Table 39. Work not done, with the effort, the likelihood it pays off and the value of finishing it.**
...
**Table 40. Phonetic misspellings against the deployed pgeo.** All five resolve, but two are weak: "Kenebunc" is found with low confidence, and "Skowhegin" ranks a venue above the town.
...
**Table 41. How coarse each phonetic encoding is on this data.** Soundex puts 18 distinct towns -- North Bancroft, Northport, North Haven, Northfield and fourteen more -- into one bucket.
...
**Table 42. Questions asked during the project, with short answers and the sections that answer them in full.**
...
**Table 43. pgeo tables (including their indexes).**
