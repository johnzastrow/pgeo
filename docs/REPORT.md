...
pgeo 0.7.0, project 0.12.2, commit `59e7c59 (with uncommitted changes)` ·
...
- [1. Introduction](#1-introduction)
  ...
  - [1.4 Other geocoders that live in PostgreSQL](#14-other-geocoders-that-live-in-postgresql)
- [2. Methods](#2-methods)
...
**Table 2. Geocoders whose query logic runs inside PostgreSQL, surveyed September 2026.**
...
**Table 3. HTTP options for serving the SQL API.**
...
**Table 4. Data sources, raw input sizes, and what each engine holds from them.** Pelias counts are Elasticsearch documents; pgeo counts are rows in its feature table.
...
**Table 6. pgeo build steps and their duration (latest build).**
...
435 MB Elasticsearch snapshot or a PostgreSQL dump. Table 24 in Section 3.5
...
**Table 7. Composition of the accuracy test set by endpoint, category and query quality.**
...
**Table 8. Fuzz levels.** House numbers are never corrupted, so every query stays answerable.
...
**Table 9. Load-test query corpus (drawn at random per request).**
...
**Table 10. Example requests from the load session.**
...
**Table 11. Latency targets (95th percentile) used as the pass criterion.**
...
large (Table 9) and drawn at random, so exact repeats are rare.
...
**Table 12. Tested configurations.** pgeo configurations were run with each front end (rest = pure SQL through PostgREST, api = FastAPI); budgets include 0.8 GB for the operating system.
...
**Table 13. Accuracy on 1,560 cases.** Correct = right answer at rank 1 (misses: nothing, or low confidence); Hit@5 = right answer in the top five; confidence columns are the mean confidence of the first result when right and when wrong.
...
**Table 14. Accuracy by category (all query qualities).**
...
pgeo (Table 16). Eight "misses" are test-set errors (Death Valley in
...
**Table 16. Remaining pgeo failures by category.**
...
**Table 17. Tuning changes, the problem each solved, and the measured effect.**
...
**Table 18. Median and p95 latency at 3 users (ms).**
...
missing admin-join index was added; it now takes 5-18 ms (Table 21).
...
**Table 19. Pelias capacity by configuration.**
...
**Table 20. pgeo capacity by configuration (after the reverse-geocoding index fix).**
...
**Table 21. Effect of the reverse-geocoding index on pgeo capacity and reverse latency.**
...
**Operation.** Table 22 turns the capacity results into a sizing guide: for a
...
**Table 23. Operating requirements of each platform (Maine data).**
...
never needs the raw data or the build tools. Table 24 gives the measured
cost of a full build of each engine.

**Table 24. Build resources:** a full build of each engine, measured with `tests/build/measure_build.py` (wall time, peak memory of all build containers and processes, average and peak CPU, disk used by the result).
...
**Table 26. Pelias by dataset:** D1 Who's On First; D2 + OpenAddresses; D3 + OpenStreetMap; D4 + GNIS and ZCTA; D5 + Overture places (full).
...
**Table 27. Compatibility contract cases.**
...
**Table 28. Features of each platform as deployed here.**
...
**Table 29. Feature parity through the documented Pelias API.**
...
**Table 30. Accuracy parity by category:** Pelias against pgeo (pure SQL), all query qualities. Pelias leads only where both are near perfect.
...
**Table 31. Throughput parity:** most concurrent users within all latency targets at each CPU size, with each configuration's memory budget.
...
**Table 32. Both engines on VM 120 through the production HTTPS path:** the three-user validation, the p95 per endpoint and how far the ramp went.
...
conditions (Table 31). Both engines do better on the VM than on the
...
it will fail too. Table 33 is the whole path, pass and fail.

**Table 33. Minimum-server search:** every trial, in order, for both engines. Memory totals include 0.8 GB for the operating system.
...
> **Key result.** The three-user service fits in 1.40 GB on pgeo and 7.17 GB on Pelias -- a factor of five. Both engines idle comfortably below a quarter of a core, so at this scale the machine is chosen by memory, not by CPU -- which is why the two platforms land in different price classes (Table 25).
...
**Table 34. The measured floor confirmed on a temporary VM:** the three-user validation and the ramp to the first configuration outside a latency target.
...
**Table 35. Security properties of each platform as deployed.** Measured 2026-09-20.
...
**Table 36. Which tool for which scenario.**
...
**Table 37. Questions asked during the project, with short answers and the sections that answer them in full.**
...
**Table 38. pgeo tables (including their indexes).**
