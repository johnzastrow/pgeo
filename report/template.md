...
{{callout:note|When this study was first written up, neither engine authorized anything: any device
that could reach the edge could resolve any address, which for the LANCER use case meant any
device on the LAN could resolve a client address. That was the one place where "fewest
components" and good security genuinely conflicted, and the lightest honest answer -- hashed API
keys checked at the edge, issued out of band -- was proposed as costing no new component. It is
now done (Section 3.13.2), and it cost none.}}
...
Nominatim{{cite:nominatim}} is the OpenStreetMap geocoder -- the one that answers
...
{{table:nominatim_capacity|Concurrent users each engine holds inside the latency targets, on the
same two cores. Nominatim lands between Photon and pgeo, and closer to pgeo.}}
...
{{table:nominatim_latency|Autocomplete p95 on two cores, against the 250 ms target. Nominatim has
no autocomplete endpoint, so these are full searches on a growing prefix.}}
...
{{table:nominatim_accuracy|Accuracy on the Maine test set. The right-hand column is the important
one: Nominatim's errors are overwhelmingly empty answers rather than wrong ones.}}
...
{{table:nominatim_kind|Accuracy by feature kind. Nominatim's median error on addresses is zero
metres: when it answers, it is exact. Its failures are almost entirely recall.}}
...
{{callout:note|Nominatim is the fairest comparison in this report for pgeo's architecture -- both
put the work in PostgreSQL -- and pgeo comes within 1.33 times of it on capacity while scoring
95.8% against 61.7% on this test set. That accuracy gap is mostly data rather than engine:
Nominatim indexes OpenStreetMap alone, where pgeo carries OpenAddresses, Overture, GNIS, Who's On
First and ZCTA. The part that is not data is the missing autocomplete endpoint, and that is a
real architectural difference rather than a matter of loading more sources.}}
...
{{table:four_engines|The four engines measured in this study, on the same hardware, data and
test set. Sources and endpoints explain more of the accuracy column than engine quality does.}}
